...
public class BufferWithCapacityTest {

    private Buffer<Integer> buffer;
    private ExecutorService executorService;
    private Runnable p1;
    private Runnable p2;
    private final List<Integer> preparedData1 = Stream
            .generate(() -> (int) (Math.random() * 100))
            .limit(20)
            .toList();

    private final List<Integer> preparedData2 = Stream
            .generate(() -> (int) (Math.random() * 100))
            .limit(20)
            .toList();
    ...
    public void oneConsumerOneProducer(){
        buffer = new BufferWithCapacity<>();
        p1 = new Producer<>(preparedData1, 100, buffer);
    ...
    public void fastProducersWaitSlowConsumer(){
        buffer = new BufferWithCapacity<>();
        p1 = new Producer<>(preparedData1, 50, buffer);
        p2 = new Producer<>(preparedData2, 20, buffer);
        Callable<List<Integer>> c1 = () -> {
            try{
                List<Integer> received = new ArrayList<>();
    ...
    public void bufferHasCapacityJustForOneElement(){
        buffer = new BufferWithCapacity<>(1);
        p1 = new Producer<>(preparedData1, 50, buffer);
        p2 = new Producer<>(preparedData2, 20, buffer);
